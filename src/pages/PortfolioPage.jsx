...
//   const [title, setTitle] = useState('');
//   const [description, setDescription] = useState('');
//   const [items, setItems] = useState([{ title: '', category: '', image: '', description: '' }]);
//   const [uploading, setUploading] = useState(false);
...
//       // Store the raw API data without transformation
...
//       setTitle('');
//       setDescription('');
//       setItems([{ title: '', category: '', image: '', description: '' }]);
//     }
//     setModalVisible(true);
//     console.log('Modal state:', { modalMode: mode, modalVisible: true });
//   };
...
//     if (modalMode !== 'create' && modalMode !== 'edit') return;
//     if (!title.trim() || items.some(item => !item.title.trim() || !item.category.trim())) {
//       Alert.alert('Error', 'Please fill in all required fields (title, item title, and category).');
//       return;
//     }
...
//   const addItem = () => {
//     setItems([...items, { title: '', category: '', image: '', description: '' }]);
//   };
...
//                 </View>
//                 <View className="flex-row flex-wrap justify-between">
//                   {portfolioData.items?.map((item, index) => renderProjectCard(item, index))}
//                 </View>
//               </View>
//             </View>
//           </>
...
//                           <TextInput
//                             className="bg-white border border-gray-300 rounded-lg p-3 text-sm text-gray-900 mb-3"
//                             placeholder="e.g., A collection of wedding photos..."
//                             placeholderTextColor="#9CA3AF"
//                             value={item.description}
//                             onChangeText={(text) => handleItemChange(itemIndex, 'description', text)}
//                             multiline
//                             numberOfLines={3}
//                             textAlignVertical="top"
//                             style={{ minHeight: 80 }}
...
//                                 className="w-full h-32 rounded-lg"
//                                 resizeMode="cover"
//                                 onError={(e) => console.log(`Failed to load modal image: ${item.image}`, e.nativeEvent.error)}
//                               />
//                               <TouchableOpacity
//                                 className="absolute top-2 right-2 bg-red-500 p-2 rounded-full"
//                                 onPress={() => handleItemChange(itemIndex, 'image', '')}
...
const PortfolioPage = () => {
  ...
  const renderProjectCard = (item, index) => (
    <View key={index} className="w-[48%] mb-4">
      <View className="bg-white rounded-2xl shadow-lg overflow-hidden border border-orange-100" style={{ height: 220 }}>
        <LinearGradient colors={['#FB923C', '#F97316']} className="rounded-2xl overflow-hidden flex-1">
          {item.image ? (
            <Image
              source={{ uri: item.image }}
              className="w-full h-32"
          ...
          <View className="p-4 flex-1 justify-between">
            <View>
              <View className="bg-white bg-opacity-20 rounded-xl px-2 py-1 self-start mb-2">
                <Text className="text-orange-400 text-xs font-semibold">{item.category}</Text>
              </View>
              <Text className="text-white text-sm font-bold mb-1">{truncateTitle(item.title)}</Text>
            </View>
            {item.description && (
              <Text className="text-white text-xs leading-4 opacity-90" numberOfLines={2}>
                {item.description}
